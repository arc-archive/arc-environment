...
export class EnvironmentSelectorElement extends VariablesConsumerMixin(LitElement) {
  static get styles() {
  ...
  [listOptionsTemplate]() {
    ...
    return html`
    <anypoint-icon-item data-id="">Default</anypoint-icon-item>
    ${environments.map((item) => html`<anypoint-icon-item data-id="${item._id}">${item.name}</anypoint-icon-item>`)}
    <div class="separator"></div>
    <anypoint-icon-item data-action="add-environment">
      <arc-icon slot="item-icon" icon="addCircleOutline"></arc-icon>
      Add environment
    </anypoint-icon-item>
    ${environment ? html`
    <anypoint-icon-item data-action="remove-environment">
